    ...
    return (
        <div>
            <div className="flex justify-between mb-1">
                <span className="text-base font-medium text-brand-primary">{title}</span>
                <span className="text-sm font-medium text-brand-primary">{Math.round(value)}% Done</span>
            </div>
...
export default function PillarDetailPage({ pillar, supabase, user }) {
    ...
    const visual = PILLAR_VISUALS[pillar]; // Ambil data visual untuk pilar saat ini
            ...
            {loading ? <p className='text-center'>Memuat data progres...</p> : (
                <div className="space-y-10">
                    {Object.entries(groupedCriteria).map(([subSectionTitle, data]) => {
                        ...
                        return (
                            <div key={subSectionTitle} className="bg-brand-surface p-6 rounded-2xl shadow-md border">
