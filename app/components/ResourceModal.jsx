...
export default function ResourceModal({ isOpen, onClose, onSave, supabase, user, resourceToEdit }) {
    ...
    const [formData, setFormData] = useState({
        ...
        image_url: '',
        duration: '',
        is_published: true, // Tambahkan status publikasi
    });
        ...
        if (isOpen) {
            if (isEditMode && resourceToEdit) {
                ...
                setFormData({
                    ...
                    resource_url: '', image_url: '', duration: '', is_published: true
                });
            }
        }
    ...
    const handleFormSubmit = async (publishAction) => {
        ...
        const dataToSubmit = {
            ...formData,
            created_by: user.id,
            is_published: publishAction === 'publish', // true jika 'publish', false jika 'draft'
    ...
    // Mencegah modal tertutup saat form belum selesai diisi (mencegah refresh)
    // Walaupun tidak bisa 100% mencegah refresh halaman, ini mencegah penutupan modal yang tidak disengaja.
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={handleOverlayClick}>
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="overflow-y-auto">
                            <main className="p-8 space-y-4">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" value={formData.description} onChange={handleChange} rows="3" className="w-full p-2 border border-slate-300 rounded-lg"></textarea>
                                </div>
                                <div>
