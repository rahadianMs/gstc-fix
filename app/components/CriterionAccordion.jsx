...
export default function CriterionAccordion({ criterion, user, supabase, onDataChange }) {
    ...
    // Fungsi fetch yang akan kita panggil secara manual
    ...
    const handleSendMessage = async () => {
        ...
        if (!error) {
            setNewMessage('');
            await fetchMessages(); // Panggil fetch ulang setelah mengirim
        }
        setIsSending(false);
    };
    ...
    const handleDeleteMessage = async (messageId) => {
        ...
        if (isConfirmed) {
            ...
            if (!error) {
                await fetchMessages(); // Panggil fetch ulang setelah menghapus
            }
        }
    };
    ...
    useEffect(() => { const allStatuses = subIndicators.map(sub => evidenceMap[sub.id]?.status || 'To Do'); if (allStatuses.length === 0) { setOverallStatus('To Do'); return; } if (allStatuses.every(s => s === 'Done')) { setOverallStatus('Done'); } else if (allStatuses.some(s => s !== 'To Do')) { setOverallStatus('In Progress'); } else { setOverallStatus('To Do'); } }, [evidenceMap, subIndicators]);
    ...
    return (
        <>
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden transition-shadow hover:shadow-md">
                 <button onClick={() => setIsOpen(!isOpen)} className="w-full p-6 text-left flex justify-between items-center hover:bg-slate-50/50 transition-colors"><div className="flex items-start gap-5"><div className="flex-shrink-0 w-14 h-14 bg-slate-100 border border-slate-200 rounded-xl flex items-center justify-center transition-colors"><span className="font-bold text-lg" style={{color: '#3f545f'}}>{criterion.criterion_code}</span></div><div><h4 className="text-lg font-bold text-slate-800">{criterion.criterion_title}</h4><p className="text-sm text-slate-500 mt-1 max-w-3xl">{criterion.criterion_description}</p></div></div><div className='flex items-center gap-4'><span className={`px-3 py-1 text-xs font-bold rounded-full ${statusInfo.style}`}>{statusInfo.label}</span><ChevronDownIcon className={`w-6 h-6 text-slate-400 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} /></div></button>
                <AnimatePresence>
                    {isOpen && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden" transition={{ duration: 0.3, ease: "easeInOut" }}>
                            <div className="grid grid-cols-1 lg:grid-cols-5 border-t border-slate-200/80">
                                <div className="lg:col-span-3 p-8"><h5 className="text-base font-bold text-slate-800 mb-2">Indicators</h5>{subIndicators.length > 0 ? subIndicators.map(sub => (<SubIndicatorItem key={sub.id} sub={sub} evidence={evidenceMap[sub.id]} onLinkEvidence={() => handleOpenEvidenceModal(sub)} onViewReview={() => handleOpenReviewModal(evidenceMap[sub.id])} />)) : <p className="text-sm text-slate-400">Memuat indikator...</p>}</div>
                                <div className="lg:col-span-2 p-8 bg-slate-50 lg:border-l border-slate-200/80 flex flex-col">
                                    ...
                                    <div className="flex-grow space-y-4 h-64 overflow-y-auto mb-4 border-b pb-4">
                                        {discussionMessages.map(msg => {
                                            ...
                                            return (<div key={msg.id} className={`group flex flex-col ${isMyMessage ? 'items-end' : 'items-start'}`}><div className={`relative p-3 rounded-lg max-w-xs ${isMyMessage ? 'bg-blue-100' : 'bg-white border'}`}><div className="font-bold text-sm" style={{color: '#3f545f'}}>{isMyMessage ? 'Anda' : senderName}</div><p className="text-sm text-slate-700">{msg.message}</p>{isMyMessage && (<button onClick={() => handleDeleteMessage(msg.id)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity p-1 text-red-500 hover:bg-red-100 rounded-full"><TrashCanIcon className="w-4 h-4" /></button>)}</div><p className="text-xs text-slate-400 mt-1">{new Date(msg.created_at).toLocaleString('id-ID', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p></div>);
                                        })}
