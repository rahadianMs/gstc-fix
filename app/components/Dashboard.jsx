...
export default function Dashboard({ supabase, user, activeDashboardPage, setActiveDashboardPage, isUserMenuOpen, setIsUserMenuOpen, userMenuRef, handleLogout }) {
    ...
    if (userRole === 'consultant') {
        ...
        sidebarLinks = [
            ...
            { 
                ...
                children: [
                    { id: 'compliance-a', text: 'PILAR A: Pengelolaan Berkelanjutan' },
                    { id: 'compliance-b', text: 'PILAR B: Keberlanjutan Sosial-Ekonomi' },
                    { id: 'compliance-c', text: 'PILAR C: Keberlanjutan Budaya' },
                    { id: 'compliance-d', text: 'PILAR D: Keberlanjutan Lingkungan' },
                ] 
            },
            { id: 'pembelajaran', text: 'Pembelajaran', icon: <AcademicCapIcon /> },
            { id: 'panduan', text: 'Panduan', icon: <QuestionMarkCircleIcon /> },
    ...
    return (
        <div id="app-wrapper" className="flex min-h-screen bg-brand-background">
            <aside className="fixed top-0 left-0 z-40 flex flex-col h-screen p-6 w-72 text-white bg-brand-primary">
               <div className="pb-6 mb-4 border-b border-white/10">
                    <h1 className="text-xl font-bold"><span className="text-brand-accent">wise steps</span> consulting</h1>
               </div>
                <nav className="flex flex-col flex-grow gap-1 overflow-y-auto">
                    {sidebarLinks.map(link => {
                        ...
                        return link.children ? (
                            <div key={link.id}>
                                <button onClick={() => setIsComplianceOpen(!isComplianceOpen)} className={`w-full flex items-center justify-between gap-4 p-3 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-black/20 text-white font-semibold' : 'text-white/80 hover:bg-black/10 hover:text-white'}`}>
                                ...
                                <AnimatePresence>{isComplianceOpen && ( <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-8 mt-1 space-y-1">
                                    {link.children.map(child => (
                                        <button key={child.id} onClick={() => setActiveDashboardPage(child.id)} className={`w-full text-left py-2 px-3 rounded-md text-sm transition-colors ${activeDashboardPage === child.id ? 'text-white font-bold bg-black/20' : 'text-white/70 hover:text-white hover:bg-black/10'}`}>{child.text}</button>
                                    ))}
                                </motion.div>)}</AnimatePresence>
                            </div>
                        ) : (
                            <button key={link.id} onClick={() => setActiveDashboardPage(link.id)} className={`flex items-center gap-4 p-3 rounded-lg text-sm font-medium transition-colors ${isActive ? 'bg-black/20 text-white font-semibold' : 'text-white/80 hover:bg-black/10 hover:text-white'}`}>{link.icon}<span>{link.text}</span></button>
                        );
                    })}
                </nav>
            ...
            <div className="flex flex-col flex-1 w-full ml-72">
                <header className="sticky top-0 z-30 flex items-center justify-between h-20 px-10 bg-brand-primary border-b border-black/20">
                    <h2 className="text-2xl font-bold text-white">{pageTitle}</h2>
                    <div className="relative" ref={userMenuRef}>
                       <button onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className="flex items-center justify-center w-10 h-10 rounded-full text-slate-300 hover:bg-white/10 hover:text-white">
                            <UserCircleIcon />
                        </button>
                        <AnimatePresence>
                        {isUserMenuOpen && (
                            <motion.div 
                                ...
                                exit={{ opacity: 0, y: -10 }}
                                className="absolute right-0 w-48 mt-2 origin-top-right bg-brand-surface rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                            >
                                <div className="py-1">
